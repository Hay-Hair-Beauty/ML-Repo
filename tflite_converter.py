...
def define_flags():
  flags.DEFINE_string("model_file", "hay_model.tflite",
                      "Path and file name to the TFLite model file.")
  flags.DEFINE_string("label_file", "label.txt", "Path to the label file.")
  flags.DEFINE_string("export_directory", "/result/",
...
_MODEL_INFO = {
    ...
    "hay_model.tflite":
        ModelSpecificInfo(
            ...
            version="v1",
            image_width=64,  # Replace with your model's input width
            image_height=64,  # Replace with your model's input height
            image_min=0,
            image_max=255,
...
def main(_):
    ...
    if model_basename not in _MODEL_INFO:
        raise ValueError("The model info for, {0}, is not defined yet.".format(model_basename))

    export_model_path = os.path.join(FLAGS.export_directory, model_basename)

    # Remove the existing file if it exists
    if tf.io.gfile.exists(export_model_path):
        tf.io.gfile.remove(export_model_path)

    # Copies model_file to export_path.
    tf.io.gfile.copy(model_file, export_model_path, overwrite=True)  # Ensure overwrite is set to True
